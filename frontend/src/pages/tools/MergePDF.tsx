  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 sm:py-10">
      <div className="text-center mb-6 sm:mb-8">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-slate-100 mb-2">Merge PDF Files</h1>
        <p className="text-slate-500 dark:text-slate-400">Combine multiple PDF files into a single document</p>
      </div>

      {status !== 'done' && (
        <div className="space-y-4">
          <DropZone onFilesAdded={addFiles} multiple label="Tap to select or drop PDF files" />
          {files.length > 0 && (
            <>
              {files.length > 1 && (
                <p className="text-xs text-slate-400 text-center">
                  Use the arrows to reorder files before merging
                </p>
              )}
              <FileList
                files={files}
      ...
      {status === 'done' && result && (
        <div className="text-center space-y-4 py-8">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <Button variant="secondary" onClick={reset} className="w-full sm:w-auto">Merge More</Button>
          </div>
        </div>
      )}
